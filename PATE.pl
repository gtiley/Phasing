...
--runmode STRING <serial or cluster or alleles>
--template STRING <template file for distributing on a cluster>
--genotype INT <cluster or iupac>
...
\n/*--------FLAG OPTIONS--------*/\n
--runmode
	serial = run each step serially (a string is still needed for --template although it is not used)
	cluster = distribute each sample seperatley on a cluster with directives given in template.sh
	alleles = Use phasing information to make orthologous fasta files with haplotype sequences AFTER --runmode serial or cluster
...
elsif (scalar(@ARGV) > 0)
{
    for my $i (0..(scalar(@ARGV) - 1))
    {
		...
		if ($ARGV[$i] eq "--runmode")
		{
	    	...
	    	if (($passedArgs{runmode} ne "serial") and ($passedArgs{runmode} ne "cluster") and ($passedArgs{runmode} ne "alleles"))
	    	{
...
foreach my $tax (sort keys %taxaPloidy)
{
	my @fq1 = ();
	my @fq2 = ();
	if ($passedArgs{runmode} eq "serial")
    {
...
#		print "STEP 6: Cleaning up unecessary reference files and alignments used for genotyping\n";
    ...
    elsif ($passedArgs{runmode} eq "cluster")
    {
		@fq1 = glob("$controlArgs{FQ}/$tax.R1.*");
		@fq2 = glob("$controlArgs{FQ}/$tax.R2.*");
		system "mkdir $controlArgs{GENOTYPE_OUT}/$tax";
